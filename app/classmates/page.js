...
import getClassMates from '@/lib/getClassMates'
			...
			{classmates.map((classmate, index) => (
				<div key={index} className="w-full sm:w-1/2 md:w-1/5 p-4">
					<div className="flex flex-col items-center bg-white shadow-lg p-4 rounded-lg">
						<Image 
							src={'https://q.qlogo.cn/g?b=qq&nk='+classmate.qq+'&s=100'} 
							alt={classmate.name} 
							width={96}
							height={96}
						...
						<div className="flex items-center justify-center mb-2">
							...
							<p className="text-lg font-bold inline-flex items-center">{classmate.id}{' '}{classmate.name}</p>
						</div>
					</div>
				</div>
			))}
